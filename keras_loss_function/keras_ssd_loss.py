...
class SSDLoss:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        classification_loss = tf.cast(self.log_loss(y_true[:,:,:-12], y_pred[:,:,:-12]), dtype=tf.float32) # BY SHANTANU
        localization_loss = tf.cast(self.smooth_L1_loss(y_true[:,:,-12:-8], y_pred[:,:,-12:-8]), dtype=tf.float32) # BY SHANTANU
        ...
        positives = tf.cast(tf.reduce_max(y_true[:,:,1:-12], axis=-1), dtype=tf.float32 ) # BY SHANTANU
        ...
        total_loss = total_loss * tf.cast(batch_size, dtype=tf.float32) # BY SHANTANU
